...
new const g_szGlitchNames         [CAC_GLITCH__ALL][] = {"Quick reload", "Fast fire", "Fast move", "C-Bug", "Fast sprint", "Quick stand"};
new const g_szGameOptionNames [CAC_GAMEOPTION__ALL][] = {"Vehicle blips", "Manual reloading", "Drive on water", "Fireproof", "Sprint", "Infinite sprint", "Infinite oxygen", "Infinite ammo", "Night vision", "Thermal vision"};
new const g_szCheatNames           [CAC_CHEAT__ALL][] = {"Aimbot (1)", "Aimbot (2)", "Triggerbot (1)", "Triggerbot (2)", "Nametag wallhack (1)", "ESP wallhack (1)", "Macro keybind (1)", "Fake ping (1)", "Weapon info (1)", "No recoil (1)", "No recoil (2)", "Aimbot (3)", "Aimbot (4)", "CLEO"};
...
public CAC_OnCheatDetect(player_id, cheat_id, opt1, opt2)
{
	...
	format(string, sizeof(string), "* {EE5555}Cheat detected: {FFFFFF}Player: %s [ID: %i] | Cheat: %s [ID: %i] | Opt1: 0x%x | Opt2: 0x%x", playerName, player_id, g_szCheatNames[cheat_id], cheat_id, opt1, opt2);
...
public CAC_OnPlayerKick(player_id, reason_id)
{
    ...
    if(IsPlayerConnected(player_id))
        GetPlayerName(player_id, playerName, sizeof(playerName));
	format(string, sizeof(string), "* {EE5555}Player kicked: {FFFFFF}Player: %s [ID: %i] | Reason: %s [ID: %i]", playerName, player_id, g_szKickReasons[reason_id], reason_id);
	SendClientMessageToAll(-1, string);
    print(string);
    return 1;
}
...
COMMAND:cac_commands(playerid, params[])
{
...
COMMAND:cac_status(playerid, params[])
{
...
COMMAND:cac_hwid(playerid, params[])
{
...
COMMAND:cac_readmemory(playerid, params[])
{
...
COMMAND:cac_glitchstatus(playerid, params[])
{
...
COMMAND:cac_glitchplayerset(playerid, params[])
{
...
COMMAND:cac_glitchtoggle(playerid, params[])
{
...
COMMAND:cac_gameoptionstatus(playerid, params[])
{
...
COMMAND:cac_gameoptionplayerset(playerid, params[])
{
...
COMMAND:cac_gameoptiontoggle(playerid, params[])
{
...
COMMAND:cac_gameresourcestatus(playerid, params[])
{
...
COMMAND:cac_gameresourcetype(playerid, params[])
{
	new type, string[128];
...
COMMAND:cac_gameresourceplayerset(playerid, params[])
{
