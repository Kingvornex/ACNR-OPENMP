...
new const g_szCheatSections[CAC_CHEAT__ALL][] = {"aimbot_1", "aimbot_2", "triggerbot_1", "triggerbot_2", "nametag_wh_1", "esp_wh_1", "macro_1", "fake_ping_1", "weaponinfo_1", "norecoil_1", "norecoil_2", "aimbot_3", "aimbot_4", "cleo"};
...
{
	if(cheat_id < 0 || cheat_id >= sizeof(g_iCheatStatus) || !g_iCheatStatus[cheat_id]) // skip if detection is disabled for this cheat type, or cheat type is invalid
		return 1;
    ...
    format(string, sizeof(string), "SAMPCAC: {FFFFFF}Cheat detected: Player: %s [ID: %i] | Cheat: %s", playerName, player_id, g_szCheatNames[cheat_id]);
...
getINIString(filename[], section[], item[], result[], max_size = sizeof(result)) {
    new File: inifile;
    new line[512];
